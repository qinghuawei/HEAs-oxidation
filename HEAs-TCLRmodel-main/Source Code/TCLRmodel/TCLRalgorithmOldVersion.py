...
def createTree(dataSet, feats, leaf_no, correlation='PearsonR(+)', minsize=3, threshold=0.95, mininc=0.01):
    ...
    if correlation == 'PearsonR(+)':
        ...
        if splitSuccess:
            ...
            #节点的每一次分割, 是如何记录节点信息的?
            node.bestFeature, node.bestValue = bestFeature, bestValue # 这行代码的作用没看明白
...
def render(label, node, dot, feats):
    ...
    if node.lc is not None:
        render(label + 'A', node.lc, dot, feats)
        render(label + 'B', node.rc, dot, feats)
        dot.edge(label, label + 'A', feats[node.bestFeature] + "≤" + str(round(node.bestValue,2)))
        dot.edge(label, label + 'B', feats[node.bestFeature] + ">" + str(round(node.bestValue,2)))
...
def start(filePath, correlation, minsize, threshold, mininc):
    ...
    dot.render('Result of TCLR {year}.{month}.{day}-{hour}.{minute}'.format(year=namey, month=nameM, day=named, hour=nameh,
                                                                     minute=namem))
